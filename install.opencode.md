...
- Managed runtime hard timeout is **12 hours** by default (per-agent override via `runtimeConfig.timeoutMs`). Managed Codex also has a 30-minute quiet-stall watchdog (`runtimeConfig.quietTimeoutMs` or `runtimeConfig.silenceTimeoutMs`) so wedged Codex turns fail cleanly instead of sitting in `running` until the hard timeout. Stuck `mcpToolCall aify-comms` turns fail faster after 90 seconds by default (`runtimeConfig.mcpToolTimeoutMs` or `runtimeConfig.commsToolTimeoutMs`; set to `0` only for debugging). Set the quiet timeout to `0` only for agents expected to run very long silent commands.
